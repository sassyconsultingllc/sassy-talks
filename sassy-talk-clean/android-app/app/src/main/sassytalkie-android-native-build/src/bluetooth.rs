...
impl BluetoothManager {
    ...
    pub fn connect(&mut self, device_address: &str) -> Result<(), String> {
        ...
        let socket = target.create_rfcomm_socket(SERVICE_UUID)?;
        socket.connect()?;
        ...
        conn.socket = Some(socket); conn.input_stream = Some(input_stream); conn.output_stream = Some(output_stream);
        conn.state = ConnectionState::Connected;
        conn.device = Some(BluetoothDevice { address: device_address.to_string(), name, paired: true });
        Ok(())
    }
