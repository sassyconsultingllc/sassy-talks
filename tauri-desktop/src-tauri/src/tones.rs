...
// CodeMark: SCLLC1-sassytalkie-LI3Y3ERE6B55
...
impl TonePlayer {
    ...
    pub async fn play(&self, tone_type: ToneType) -> Result<(), ToneError> {
        ...
        stream
            .play()
            .map_err(|e| ToneError::PlayError(e.to_string()))?;

        // Wait for playback to complete or timeout
        let _ = tokio::time::timeout(tokio::time::Duration::from_secs(2), rx).await;
